    ...
    def handle(self, text):
        if text.startswith("calories_calc "):
            ...
            for command in commands:
                ...
                if params:
                    if command["weight"]:
                        found_food[command["name"]] = {
                            name: float(value) * float(command["weight"]) / 100
            ...
            if not_found_food:
                for not_found in not_found_food:
                    results.append(
                        "Food '{}' is not found in base".format(not_found.strip())
                    )
            if found_food:
                results.append(
                ...
                for name, parameter in found_food.items():
                    results.append(
                        "{:^30}|{:^10.2f}|{:^10.2f}|{:^10.2f}|{:^10.2f}|{:^10.2f}".format(
                            name,
                            parameter["water"],
                            parameter["proteins"],
                            parameter["fats"],
